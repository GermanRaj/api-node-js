...
test.describe('User management API with loop', () => {
    ...
    test('Delete first user and recieve correct number of users', async ({request}) => {
        await createUsers(5)
        const response = await request.get(`${baseURL}`);
        const responseBody = await response.json()
        ...
        const responseDeleteZeroIndex = await request.get(`${baseURL}`);
        const responseBodyZero = await responseDeleteZeroIndex.json()
        console.log(responseDelete.json());
        console.log(responseBodyZero);
        expect.soft(responseBodyZero.length).toBe(4);
    });
